document.addEventListener("DOMContentLoaded", () => {
    ...
    function animateMenuItems(items, direction) {
      items.forEach((item, index) => {
        setTimeout(() => {
          item.style.left = direction === "in" ? "0px" : "-100%";
        }, index * 50);
      });
    }
  
    const linkElements = document.querySelectorAll(".menu-item a, .menu-item span, .menu-title p, .menu-content p");
  
    linkElements.forEach((link) => {
      SplitType.create(link, { types: "words, chars" });
    });
    ...
    document.querySelectorAll(".menu-item").forEach((item) => {
      ...
      if (linkElement) {
        ...
        item.querySelector(".menu-item-link .bg-hover").style.width = `${width + 30}px`;
        ...
        linkElement.addEventListener("mouseleave", () => clearColorChars(chars));
      }
    });
    ...
    const links = document.querySelectorAll(".menu-item, .menu-sub-item, .menu-title, .menu-sub-item .menu-content");
  
    links.forEach((link) => {
      link.addEventListener("mouseenter", (event) => {
        const targetElement = event.currentTarget.querySelector(".menu-item-link a, .menu-title p, .menu-content p");
        if (targetElement) {
